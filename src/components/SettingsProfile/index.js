...
export default function SettingsProfile({ user, userStatusCode }) {
  const [displayName, setDisplayName] = useState(user.userImpL.displayName)
  const [email, setEmail] = useState(user.userImpL.email)
        ...
        <form className="settings-main-form" onSubmit={handelSubmit}>
          ...
          <div className="input-content">
            ...
            <select onChange={(e) => setLocation(e.target.value)}>
              <option disabled selected={!location} value="">
                Select location
              </option>
              {states.map((state) => (
                <option value={state} selected={location === state}>
                  {state}
                </option>
              ))}
            </select>
